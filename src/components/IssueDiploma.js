...
const IssueDiploma = () => {
    ...
    const loadWeb3 = async () => {
      if (window.ethereum) {
        ...
        try {
          ...
          setErrorMessage("User denied account access. Please allow access to MetaMask.");
        }
      } else if (window.web3) {
  ...
  const checkAuthorizedIssuer = async (account) => {
    try {
      ...
      setErrorMessage("Error checking authorized issuer. Please check the console for more details.");
    }
  };
  ...
  const fetchPendingRequests = async () => {
    try {
      const web3 = window.web3;
      const contract = new web3.eth.Contract(diplomaNFTAbi, process.env.REACT_APP_CONTRACT_ADDRESS);
      const [ids, addresses, names, institutionIds] = await contract.methods.getPendingRequests().call();
      const requests = ids.map((id, index) => ({
        requestId: id,
        requester: addresses[index],
      ...
      setErrorMessage("Error fetching pending requests. Please check the console for more details.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage('Failed to issue diploma. Please ensure you have MetaMask installed and connected.');
    }
  };
  ...
  const handleRetrieve = async (e) => {
    ...
    try {
      ...
      setErrorMessage('Failed to retrieve diploma. Please ensure the token ID is correct.');
    }
  };
  ...
  const handleRequestAuthorization = async () => {
    ...
    try {
      ...
      console.log("Authorization request submitted:", form.requesterInstitutionName, form.requesterInstitutionID);
    } catch (error) {
      console.error('Error requesting authorization:', error);
      setErrorMessage('Failed to submit authorization request. Please ensure you have MetaMask installed and connected.');
    }
  };
  ...
  const handleVoteOnRequest = async (requestIds, approve) => {
    try {
      ...
      await fetchPendingRequests(); // Refresh the pending requests
      console.log("Voted on requests:", requestIds, "Approve:", approve);
    } catch (error) {
      console.error('Error voting on request:', error);
      setErrorMessage('Failed to vote on request. Please ensure you have MetaMask installed and connected.');
    }
  };
  ...
  const handleFilterSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Filtered diplomas:", filteredDiplomas);
    } catch (error) {
      console.error('Error fetching filtered diplomas:', error);
      setErrorMessage('Failed to fetch filtered diplomas. Please ensure you have MetaMask installed and connected.');
    }
  };
...
//     } catch (error) {
//       console.error("Error checking authorized issuer:", error);
//       setErrorMessage("Error checking authorized issuer.");
//     }
//   };
...
//     const web3 = window.web3;
//     const contract = new web3.eth.Contract(diplomaNFTAbi, process.env.REACT_APP_CONTRACT_ADDRESS);
//     const requestDetails = await contract.methods.getPendingRequests().call();
//     const requests = requestDetails[0].map((requestId, index) => ({
//       requestId,
//       requester: requestDetails[1][index],
//       institutionName: requestDetails[2][index],
//       institutionID: requestDetails[3][index],
//     }));
//     setPendingRequests(requests);
//   };
