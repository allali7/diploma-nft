...
import diplomaNFTAbi from '../DiplomaNFT.json'; // Ensure this JSON file is generated from your contract's ABI
...
const IssueDiploma = () => {
  ...
  const fetchPendingRequests = async () => {
    try {
      const web3 = window.web3;
      const contract = new web3.eth.Contract(diplomaNFTAbi, process.env.REACT_APP_CONTRACT_ADDRESS);
      const ids = await contract.methods.getPendingRequestIds().call();
      const requests = await Promise.all(ids.map(async (id) => {
        const request = await contract.methods.issuerRequests(id).call();
        return {
          requestId: id,
